...
addEventHandler("onResourceStart", resourceRoot, function()
    outputDebugString("[Jebiga Carball] Carball gamemode initialized")
end)
...
addEventHandler("weevil:carball:goal", root, function(team, scorer)
    ...
    if scorer and carballData.players[scorer] then
        ...
        exports.jebiga_core:addPlayerPoints(scorer, 10, GAMEMODE)
        exports.jebiga_core:addPlayerMoney(scorer, 20, "Carball goal")
    end
...
function endCarballRound()
    ...
    if winningTeam ~= "draw" then
        for _, player in ipairs(carballData.teams[winningTeam]) do
            if isElement(player) then
                exports.jebiga_core:addPlayerWin(player, GAMEMODE)
            end
        end
    end
