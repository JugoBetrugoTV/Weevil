--[[
    Weevil Multi-Gamemode - Login GUI
...
function handleLoginClick(button, state, mx, my)
    ...
    if currentTab == "login" then
        if mx >= panelX and mx <= panelX + panelW and my >= panelY + panelH - 25 and my <= panelY + panelH then
            hideLoginGUI()
            -- Play as guest
            outputChatBox("#FFFF00[Weevil] #FFFFFFPlaying as guest. Use /register to save your progress!", 255, 255, 255, true)
            return
        end
    end
end
...
addEventHandler(Events.Account.LOGIN_SUCCESS, root, function(message)
    ...
    hideLoginGUI()
    exports.weevil_core:showNotification("success", message, 4000)
end)

addEvent(Events.Account.LOGIN_FAILED, true)
addEventHandler(Events.Account.LOGIN_FAILED, root, function(message)
    errorMessage = message
    exports.weevil_core:showNotification("error", message, 4000)
end)
...
addEventHandler(Events.Account.LOGOUT, root, function(message)
    exports.weevil_core:showNotification("info", message, 3000)
    showLoginGUI()
end)
...
addEventHandler("onClientResourceStart", resourceRoot, function()
    ...
    setTimer(function()
        if not exports.weevil_accounts:isLoggedIn(localPlayer) then
            showLoginGUI()
        end
    end, 1000, 1)
end)
