--[[
    Jebiga Multi-Gamemode - Security Module
...
function loadBanList()
    ...
    outputDebugString("[Jebiga Security] Loaded " .. tableCount(bannedSerials) .. " banned serials")
end
...
function recordViolation(player, type, value)
    ...
    for _, admin in ipairs(getElementsByType("player")) do
        if exports.jebiga_core:isPlayerAdmin(admin, 1) then
...
addEventHandler("weevil:validateClientData", root, function(data)
    ...
    if data.money and type(data.money) == "number" then
        local serverMoney = exports.jebiga_core:getPlayerMoney(player)
